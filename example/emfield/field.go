...
func (em *EMField) color(p float64) color.Color {
	v := 0.5 * (p - em.field.Min) / (em.field.Max + 1)
	switch {
	...
	posR, posG, posB := rgb(em.field.pos)
	negR, negG, negB := rgb(em.field.neg)

	col := color.RGBA{
		R: uint8(math.Round(posR*v + negR*(1-v))),
		G: uint8(math.Round(posG*v + negG*(1-v))),
		B: uint8(math.Round(posB*v + negB*(1-v))),
		A: 255,
	}
	return col
}
